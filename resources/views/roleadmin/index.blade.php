...
@section('content')
<div class="container">
    <div class="mt-3 m-3 text-center">Selamat datang di halaman admin. Pilih menu di navbar untuk mengelola sistem.</div>

    {{-- <form action="{{ route('admin.filter_table') }}" method="GET" class="mb-3 d-flex gap-2">
        <!-- Dropdown Bulan -->
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @php $no = 1; @endphp Inisialisasi nomor untuk setiap unit
                        @foreach($karyawanList as $karyawan)
                        <tr>
                            <td>{{ $no++ }}</td>
                            <td>{{ $karyawan->name }}</td>
